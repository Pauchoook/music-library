  ...
  return (
    <div className="top-tracks">
      ...
      <ul className="top-tracks__list">
        {albumTracks && albumTracks.map((track, index) => 
          <Track key={track._id} active={active.track?._id === track._id} number={index + 1} handlerPlay={handlerPlayAlbumTracks} track={track} />
        )}
      </ul>
    </div>
  );
